...
const UI = () => {
  ...
  const populateTodoListUI = (item) => {
    ...
    todoItemCompleteBtn.addEventListener("input", (e) => {
      ...
      if (e.target.checked) {
        mainTodoList.forEach((e, i) => {
          if (e.key == current) {
            mainTodoList[i].completed = true;
        ...
        mainTodoList.forEach((e, i) => {
          if (e.key == current) {
            mainTodoList[i].completed = false;
  ...
  addTodoButton.addEventListener("click", (e) => {
    todoListDisplay.appendChild(addTodoForm());
